...
void FEM<dim>::generate_mesh(std::vector<unsigned int> numberOfElements){
  ...
  double x_min = 0., //EDIT DONE - define the left limit of the domain, etc.
    x_max = 0.03, //EDIT DONE
    y_min = 0., //EDIT DONE
    y_max = 0.08; //EDIT DONE
  ...
  for (uint i = 0; i < totalNodes ; ++i)
    {
        ...
        } else if (std::abs(nodeLocation[i][1] - 0.08)<1.e-8)
        {
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
	...
	detJ = Jacobian.determinant();
  double w = quad_weight[q1] * quad_weight[q2];
	...
	for(unsigned int A=0; A<dofs_per_elem; A++){
	  for(unsigned int B=0; B<dofs_per_elem; B++){
        ...
        Klocal[A][B] += basis_gradient(A, xi_1, xi_2)[i] 
        * invJacob[i][I] * kappa[I][J]
        * basis_gradient(B,xi_1,xi_2)[j] * invJacob[j][J] * w * detJ;
		  }
		}
	      }
	    }
