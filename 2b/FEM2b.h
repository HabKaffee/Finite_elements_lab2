  ...
  for (uint i = 0; i < totalNodes ; ++i)
    {
        ...
        } else if (std::abs(nodeLocation[i][0] - 0.04)<1.e-8)
        {
            boundary_values[i] = 310*(1.+8.*(nodeLocation[i][1] + nodeLocation[i][2]));
        }
    }
}
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
    ...
    Klocal = 0.;
    for(unsigned int q1=0; q1<quadRule; q1++){
      double xi_1 = quad_points[q1];
      for(unsigned int q2=0; q2<quadRule; q2++){
        double xi_2 = quad_points[q2];
	for(unsigned int q3=0; q3<quadRule; q3++){
    double xi_3 = quad_points[q3];
	  ...
	  detJ = Jacobian.determinant();
    double w = quad_weight[q1] * quad_weight[q2]* quad_weight[q3];
	  ...
	  for(unsigned int A=0; A<dofs_per_elem; A++){
		...
		for(unsigned int j=0;j<dim;j++){
          ...
          Klocal[A][B] += basis_gradient(A, xi_1, xi_2, xi_3)[i] 
        * invJacob[i][I] * kappa[I][J]
        * basis_gradient(B,xi_1,xi_2, xi_3)[j] * invJacob[j][J] * w * detJ;
        }
		  }
		}
	      }
